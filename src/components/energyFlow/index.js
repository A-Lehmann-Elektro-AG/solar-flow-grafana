...
export const EnergyFlow = ({ data, options }) => {
    var _a;
    const theme = useTheme();
    const fieldPosition = Number(!options.valueFirst);
    let grid = options.solarQuery;
    let pv = ((_a = data.series[1]) === null || _a === void 0 ? void 0 : _a.fields[fieldPosition].values[0]) || 0;
    ...
    useEffect(() => {
        (() => __awaiter(void 0, void 0, void 0, function* () {
            setFlowData(yield EnergyFlowCore.getNewFlowData(pv, grid));
        }))();
    }, [grid, pv]);
    const pvPoint = { x: 275, y: 200 };
    const loadPoint = { x: 100, y: 460 };
    const gridPoint = { x: 450, y: 460 };
    ...
    if (data.series.length < 1 || typeof pv !== "number" || typeof grid !== "number") {
        return (React.createElement("div", { style: { backgroundColor: "red", padding: "5%", borderRadius: "5px", width: "200px", textAlign: "center" } },
            React.createElement("h3", null, "Invalid/Missing data in one of the queries")));
    }
    return (React.createElement("div", { style: { position: "absolute", left: `${(-175 + options.xOffset) * options.zoom}px`, top: `${(-8 - options.yOffset) * options.zoom}px`, transform: `scale(${options.zoom})`, transformOrigin: 'center' } },
        React.createElement("div", null,
            React.createElement("div", { className: "line-holder", style: { position: 'absolute', bottom: '500px', left: '0px' } },
                React.createElement("svg", { width: "500", height: "500", style: { position: 'absolute', top: '50px', left: '0' }, viewBox: '0 0 500 500' },
                    React.createElement(EnergyLines, { flow: flowData.flowType, pvPoint: pvPoint, linesColor: (theme.visualization.getColorByName(options.linesColor)), loadPoint: loadPoint, gridPoint: gridPoint }))),
            React.createElement("div", { className: "line-holder", style: { position: 'absolute', bottom: '0px', left: '25px' } },
                React.createElement("div", { className: "point-holder", style: { position: 'absolute', top: '-300px', left: '150px' } },
                    React.createElement(Point, { label: "PV", showLegend: false, value: flowData.pv, style: customPoint(theme.visualization.getColorByName(options.solarColor)), icon: pvSVG })),
                React.createElement("div", { className: "point-holder", style: { position: 'absolute', top: -110, left: '50px' } },
                    React.createElement(Point, { showLegend: options.showLegend, label: "Load", value: flowData.load, style: customPoint(theme.visualization.getColorByName(options.loadColor)), icon: loadSVG }),
                    React.createElement(Point, { showLegend: options.showLegend, label: "Grid", value: flowData.grid, style: customPoint(theme.visualization.getColorByName(options.gridColor)), icon: gridSVG }))))));
};
